...
The correlation between the different 49 variables is done using the `cor` function, and the result is applied to the function from the `caret` package: findCorrelation. This function takes care to point to the variables that are more correlated with the others, as default this function has a cutoff at 0.90. This would give us a vector with the indexs of the rows that are too correlated and are going to be discarded.

In order to perform this part of the analysis, the last row has been subtracted temporarly, because it holds the key(A,B,C,D,E) of each activity as a factor variable which should not be taken into account for this.
...
The aim of the project is to be able to classify one exercise as well done or wrongly. Thus one of the best models for classification is Random Forest, due to its effectiveness, it is the one chosen for this project. Although it is usually very slow too.

```{r,eval=FALSE,echo=TRUE}
modelFit<-train(classe ~ ., data = train_fold, method = "rf")
modelFit

#This has not been runned again due to problems of time before submitting. I have copy-pasted from my console where it was runned before. (you can check it running the .R file, but it takes a while)
...
As the model seems to be very accurate and we have clean the dataset a lot to include the minimun number of variables possible (thus the model is more simple), there should be a high order of sensitivity and specifity.
...
# This was not runned either because I did not have time to run again the train function so I do not have loaded the modelFit, thus I cannot run the predict function. I have copy pasted from my console (you can check all by running the .R file.)
